...
- [3. Build and analyze the modresorts application.](#3-build-and-analyze-the-modresorts-application)
  ...
  - [3.3 Build and deploy the WebSphere application modResort](#33-build-and-deploy-the-websphere-application-modresort)
    ...
    - [3.3.2 Deploy the WebSphere application](#332-deploy-the-websphere-application)
- [4. Explore Application Modernization Accelerator](#4-explore-application-modernization-accelerator)
  ...
  - [4.2 Create an AMA data collection for the WAS applications](#42-create-an-ama-data-collection-for-the-was-applications)
    - [4.2.1 Access the AMA UI](#421-access-the-ama-ui)
    - [4.2.2 Create in AMA a new workspace and download the AMA Discovery Tool](#422-create-in-ama-a-new-workspace-and-download-the-ama-discovery-tool)
    - [4.2.3 Use the AMA Discovery Tool to analyze the installed WebSphere Applications](#423-use-the-ama-discovery-tool-to-analyze-the-installed-websphere-applications)
    - [4.2.4 Execute the AMA Discovery Tool](#424-execute-the-ama-discovery-tool)
...
- [5. Modernise the application using IBM Bob](#5-modernise-the-application-using-ibm-bob)
  - [5.1 Explore the IBM Bob installation and complete setup](#51-explore-the-ibm-bob-installation-and-complete-setup)
    ...
    - [5.1.2 Set Up IBM Bob](#512-set-up-ibm-bob)
...
* WebSphere Liberty, version 26.0.0.8, running on Java SE 21
...
### 3.3.2 Deploy the WebSphere application 

The application has not been installed to traditional WAS so far. 
...
### 4.2.2 Create in AMA a new workspace and download the AMA Discovery Tool
...
### 4.2.3 Use the AMA Discovery Tool to analyze the installed WebSphere Applications
...
### 4.2.4 Execute the AMA Discovery Tool
...
8. Scroll down to the section **`Complexity Score`** and expand the list of issues. 
...
9. Scroll down to the section **Issues**. 
    ...
    Expand any of the 7 issues and you can get more details about the issue, recommended changes and which code is impacted. 

    <kbd>![AMA_Evaluation_Assessment-modresorts4a.png](./images/media/AMA_Evaluation_Assessment-modresorts4a.png)</kbd>
 

10. Click on **View migration plan** on the top right to view the migration plan.

11. Click on **Download plan** to download the **Migration Plan** generated by AMA.
...
12. Switch to the terminal window and execute the following command to see the content of the migration bundle. 
...
13. Close the browser window containing the AMA UI.
...
2. Bob prepares the modernization
    ...
    3. Bob offers different options of application modernization. Select **Liberty Modernization** and select to **Disable Git Flow**.
...
3. Upload the Migration plan and apply automated fixes
    ...
    5. Bob has analyzed the AMA reports and knows which issues have been identified. As  next step, Bob wants to Run OpenRewrite Recipes for the automated fixes to fix the 4 issues. Click on **Approve once** to apply the automated fixes.
    ...
    Now that Bob resolved all 4 issues with automated fixes via recipes, Bob will take a look at the remaining issues and will use agentic AI to resolve the 3 remaining issues.
...
---
**NOTE (the following is not needed if you have completed Section 5: Modernise the application using IBM Bob)** 
...
3. In the installation panel:
   ...
   * Set **Target Runtime Environment** to `Jakarta EE 10`
   ...
   ![](./images/media/More_install_new_app.png)
...
- You learned how to created a Managed Liberty server via GUI or scripting
...
In this lab you followed a complete, end-to-end enterprise Java modernization journey. Here is what you learned:

- **Build and deploy a traditional WebSphere application** — You built and deployed the modresorts application on a WebSphere Application Server instance, establishing a working baseline before modernization.
- **Assess an application with IBM Application Modernization Accelerator (AMA)** — You ran the AMA Discovery Tool against a live WebSphere cell to collect application and configuration data without needing access to source code, then used AMA to generate a detailed migration plan identifying all changes required to move to Liberty.
- **Apply automated fixes with IBM Bob** — You used IBM Bob's Java Modernization workflow to apply OpenRewrite recipes, automatically resolving known migration issues.
- **Resolve remaining issues using agentic AI** — You leveraged IBM Bob's agentic AI capabilities to analyse and fix the migration issues that could not be handled by automated recipes, reviewing and approving each step of the process.
- **Validate the modernized application on Liberty** — You deployed and tested the modernized modresorts application on a local Liberty instance running on Java 21, verifying that all endpoints worked correctly before proceeding to production deployment.
- **Deploy to IBM Modernized Runtime Extension for Java (MoRE)** — You created a Managed Liberty server within an existing WebSphere ND cell and deployed the modernized application, using both the administrative console and wsadmin scripting.
- **Understand IBM's end-to-end modernization toolchain** — You experienced firsthand how AMA, IBM Bob, and MoRE work together to dramatically reduce the effort and risk of migrating legacy Java applications to a modern, manageable Liberty runtime.

---

## 🚀 Your Turn — Modernize Your Own Applications

You have seen how straightforward the modernization journey can be with the right toolchain. Now it is time to take that knowledge back to your own organization.

**Start today:**

1. **Assess your applications** — Download and run the AMA Discovery Tool against your own WebSphere environment to quickly identify which applications are good candidates for modernization and what changes will be required.
2. **Modernize with IBM Bob** — Use IBM Bob's Java Modernization workflow to apply automated fixes and let agentic AI handle the complex migration issues, so your team can focus on delivering value instead of manual code changes.
3. **Deploy to MoRE** — Bring your modernized applications into a Managed Liberty server within your existing WebSphere ND infrastructure, with no need to retrain your operations team or abandon your current tooling.

> **Ready to get started?** Visit [IBM Application Modernization Accelerator](https://www.ibm.com/docs/en/ama), [IBM Bob](https://bob.ibm.com/), and [IBM Modernized Runtime Extension for Java](https://www.ibm.com/docs/en/more) to learn more and begin modernizing your applications today.
